...
export default function OrderDetailsPage() {
  ...
  const fetchOrderDetails = async () => {
    // Kiểm tra params.id trước khi gọi
    if (!params?.id) return;
  ...
  useEffect(() => {
    fetchOrderDetails();
  }, [params?.id]); // Chỉ phụ thuộc vào id
  ...
  const getStatusBadge = (status: string, cancelReason?: string) => {
    // Chuẩn hóa status về chữ thường để so sánh chính xác
    ...
    switch (s) {
      ...
      case "cancelled": 
        const text = cancelReason === "expired" ? "Đã hủy quá hạn thanh toán" : "Đã hủy";
        return <span className="px-3 py-1 bg-rose-100 text-rose-700 font-bold rounded-full text-xs tracking-wider uppercase border border-rose-200 shadow-sm">{text}</span>;
  ...
  // Logic kiểm tra quyền hủy đơn: Status phải là pending/paid và trong vòng 24h
  const bookingTime = new Date(order.booking_date).getTime();
  const currentTime = new Date().getTime();
  const isWithin24h = (currentTime - bookingTime) <= 24 * 3600 * 1000;

  // Kiểm tra cả tiếng Anh và tiếng Việt (phòng trường hợp Backend trả về đã dịch)
  const cancelableStatuses = ["pending", "paid", "đã thanh toán", "chờ thanh toán"];
  const canCancel = cancelableStatuses.includes(order.status.toLowerCase()) && isWithin24h;
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-8 text-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div>{getStatusBadge(order.status, order.cancel_reason)}</div>
          </div>
              ...
              <div className="mt-6 p-5 bg-indigo-50 rounded-2xl border border-indigo-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                {order.guide && (
                  <div className="flex gap-2 w-full sm:w-auto">
                     <button 
                      onClick={() => router.push(`/chat?receiver_id=${order.guide?.user_id}&name=${order.guide?.full_name}`)}
                  ...
                  <div>
                    ...
                    <p className="text-xs text-rose-700">
                      {order.cancel_reason === "expired"
                        ? "Đơn đã bị hủy do quá hạn 72 giờ chưa thanh toán."
                        : "Bạn đã hủy đơn trong vòng 24h. Tiền sẽ được hoàn trong 3-5 ngày."}
                    </p>
                  </div>
                </div>
              )}
              ...
              {order.status.toLowerCase() !== 'cancelled' && (
                <div className="mt-8 bg-orange-50 rounded-2xl p-5 border border-orange-100 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-orange-700">Bạn sẽ được hoàn tiền nếu hủy trong vòng 24 giờ kể từ khi đặt đơn.</p>
                    <p className="text-xs text-orange-700">Đơn sẽ bị hủy nếu không thanh toán sau 72 giờ kể từ khi đặt.</p>
                  </div>
                </div>
              )}
            </div>
                ...
                <div className="bg-white rounded-xl p-4 border border-slate-100 text-sm space-y-3 shadow-sm mb-8">
                  ...
                  {order.payment.transaction_id && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-xs text-slate-600 bg-slate-100 px-1 py-0.5 rounded text-right max-w-[150px] truncate">
                        {order.payment.transaction_id}
                      </span>
                    </div>
                  )}
                ...
                {canCancel ? (
                  ...
                  order.status.toLowerCase() !== "pending" && (
                    <button disabled className="w-full py-4 text-center rounded-xl bg-slate-200 text-slate-400 font-bold cursor-not-allowed">
                      {order.status.toLowerCase() === 'cancelled' ? 'Đơn hàng đã hủy' : 'Hết hạn hủy miễn phí'}
                    </button>
                  )
                )}
              </div>
