...
function CustomerChatContent() {
  ...
  const [isTyping, setIsTyping] = useState(false); // Trạng thái chờ AI
  ...
  // Lấy Session ID (Dùng chính token để lịch sử mất khi đăng xuất)
  const getSessionId = () =>{ return localStorage.getItem("user_id") || "guest";};

  // 1. Khởi tạo Socket và Load danh sách Partners (Lọc bỏ chính mình)
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/chat/partners", { 
      ...
      // FIX ROOM MA: Loại bỏ chính mình khỏi danh sách
  ...
  // 2. Lắng nghe tin nhắn từ Socket
  useEffect(() => {
    if (!socketRef.current || activeTab === "AI") return;
    const handleReceive = (data: any) => {
      const myId = Number(localStorage.getItem("user_id"));
      if (Number(data.sender_id) === myId) return; // Chặn tin lặp từ chính mình
  ...
  // 3. Load lịch sử chat (Phân tách AI và HDV)
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (activeTab === "AI") {
  ...
  const handleSend = async () => {
    ...
    // Vẽ ngay tin nhắn của mình lên UI
    setMessages(prev => [...prev, { sender_id: currentUser.id, content }]);

    if (activeTab === "AI") {
      setIsTyping(true); // Hiện hiệu ứng 3 dấu chấm
      try {
        ...
        <div className="w-1/4 bg-gray-50 border-r border-gray-200 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            <div onClick={() => setActiveTab("AI")} className={`p-4 cursor-pointer flex items-center gap-3 transition ${activeTab === "AI" ? "bg-emerald-100 border-l-4 border-emerald-500" : "hover:bg-emerald-50"}`}>
              ...
              <div><p className="font-bold text-gray-800">Trợ lý AI</p><p className="text-xs text-gray-500 italic">Phiên hiện tại</p></div>
            </div>
        ...
        <div className="flex-1 flex flex-col bg-[#f0f2f5]">
          ...
          <div className="p-4 bg-white border-t border-gray-200">
            <form onSubmit={e => { e.preventDefault(); handleSend(); }} className="flex gap-2">
              <input type="text" className="flex-1 p-3 bg-gray-100 rounded-2xl border-none focus:ring-2 focus:ring-blue-500 outline-none transition-all" value={inputMsg} onChange={e => setInputMsg(e.target.value)} placeholder="Nhập câu hỏi tại đây..." />
