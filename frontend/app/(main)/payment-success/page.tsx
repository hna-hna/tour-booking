...
function SuccessContent() {
  ...
  const tourId = searchParams.get("tourId"); // VNPay/Stripe có thể truyền thêm

  const [count, setCount] = useState(15);
  const [isConfirmed, setIsConfirmed] = useState(false); // Tránh gọi API nhiều lần
  const [tour, setTour] = useState<any>(null);

  // 0. LẤY THÔNG TIN TOUR NẾU CÓ TOUR ID
  useEffect(() => {
    if (tourId) {
  ...
  if (redirectStatus === "succeeded") {
    content = (
      <>
        ...
        {/* THÔNG TIN TOUR */}
        {tour && (
          <div className="bg-white border border-gray-100 shadow-sm p-4 rounded-2xl mb-6 flex flex-col md:flex-row gap-4 items-center md:items-start text-left">
            ...
            <div className="flex-1">
              ...
              <div className="mt-2 text-sm text-gray-500 flex flex-wrap gap-x-4 gap-y-1">
                ...
                <p>💰 {Number(tour.price).toLocaleString()}đ / người</p>
              </div>
            </div>
          </div>
        )}
