      ...
      axios.get("http://127.0.0.1:5000/vnpay_return", { params })
        .then((res) => {
          if (res.data.RspCode === "00") {
            ...
            // Lưu ngày khởi hành (trả về từ database backend)
            if (res.data.bookingDate) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-4">
      <div className="bg-white max-w-md w-full rounded-3xl shadow-2xl overflow-hidden relative">
        ...
        <div className="p-8 text-center">
          ...
          {/* THÔNG TIN TOUR (Rút gọn) */}
          {tour && (
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-6 mb-8 text-sm text-left">
              <h3 className="font-bold text-gray-800 text-base mb-2">{tour.name}</h3>
              <div className="space-y-2">
                <div className="flex justify-between items-center pb-2 border-b border-gray-200 border-dashed">
                  ...
                  <span className="font-bold text-gray-900">{bookingDate ? new Date(bookingDate).toLocaleDateString('vi-VN') : "Đang cập nhật"}</span>
                </div>
                <div className="flex justify-between items-center pb-2 border-b border-gray-200 border-dashed">
                  <span className="text-gray-500">Số lượng khách</span>
                  <span className="font-medium text-gray-900">{guests ? guests : "N/A"}</span>
                </div>
                <div className="flex justify-between items-center pb-2 border-b border-gray-200 border-dashed">
                  <span className="text-gray-500">Thời gian tour</span>
                  <span className="font-medium text-gray-900">{tour.duration} Ngày</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-500">Mức giá</span>
                  <span className="text-emerald-600 font-medium">{formatCurrency(tour.price)}/khách</span>
                </div>
              </div>
            </div>
          )}
